...
module tb_rv32i_pipeline_core_phase6_branch_predictor;
    logic clk;
    logic rst_n;
    logic [31:0] pc_dbg;
    ...
    rv32i_pipeline_core #(
        .IMEM_HEX("tests/phase6_branch_predictor.hex"),
        .BP_MODE(1)
    ) dut (
        .clk(clk),
        .rst_n(rst_n),
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            $fwrite(trace_fd, "BP: mode=SIMPLE ghr=%0d index=%0d pred_taken=%0d pred_target=%08x actual_taken=%0d actual_target=%08x mispredict=%0d\n\n", trace_bp_ghr, trace_bp_index, trace_bp_pred_taken, trace_bp_pred_target, trace_bp_actual_taken, trace_bp_actual_target, trace_bp_mispredict);
            cycle <= cycle + 1;
        end
    end
    ...
    initial begin
        $dumpfile("sim/phase6_branch_predictor.vcd");
        $dumpvars(0, tb_rv32i_pipeline_core_phase6_branch_predictor);
        report_fd = $fopen("reports/sim/branch_prediction_report.log", "w");
        trace_fd = $fopen("reports/sim/branch_prediction_trace.log", "w");
        ...
        expect_reg(5'd1, 32'h00000004);
        expect_reg(5'd2, 32'h00000004);
        ...
        expect_reg(5'd11, 32'h0000000b);

        if (bp_total_branches !== 32'd6) begin
            $display("FAIL: expected 6 resolved branches, got %0d", bp_total_branches);
            errors = errors + 1;
        end else begin
            $display("PASS: resolved branches = %0d", bp_total_branches);
        end
        expect_counter_nonzero("pred_taken_count", bp_pred_taken_count);
        expect_counter_nonzero("pred_not_taken_count", bp_pred_not_taken_count);
        expect_counter_nonzero("actual_taken_count", bp_actual_taken_count);
        expect_counter_nonzero("actual_not_taken_count", bp_actual_not_taken_count);
        expect_counter_nonzero("correct_count", bp_correct_count);
        expect_counter_nonzero("mispredict_count", bp_mispredict_count);

        accuracy = (bp_total_branches == 0) ? 0 : ((bp_correct_count * 100) / bp_total_branches);
        $display("Branch prediction accuracy: %0d%% (%0d/%0d)", accuracy, bp_correct_count, bp_total_branches);

        $fwrite(report_fd, "Phase 6 Branch Prediction Report\n");
        $fwrite(report_fd, "Predictor: 1-bit PC-indexed table\n");
        $fwrite(report_fd, "Table entries: 16\n");
        $fwrite(report_fd, "Index: PC[5:2]\n");
        ...
        if (errors == 0) begin
            $display("PASS: Phase 6 branch predictor directed test passed");
        end else begin
            $display("FAIL: Phase 6 branch predictor directed test failed with %0d error(s)", errors);
        end
